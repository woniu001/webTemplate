 ...
 * @version 创建时间：2016年7月12日 上午9:51:05
 * 
 */
public class IpUtils {
	...
	public static int getHostIP() {
		...
		if (IPArray.length == 4) {
			IP = (int) (Integer.valueOf(IPArray[0]) * Math.pow(256, 3)
					+ Integer.valueOf(IPArray[1]) * Math.pow(256, 2)
					+ Integer.valueOf(IPArray[2]) * Math.pow(256, 1) + Integer
					.valueOf(IPArray[3]) * Math.pow(256, 0));
		}
	...
	public static String longToIp(long v) {
		return String.format("%d.%d.%d.%d", (v >> 24) & 0xFF, (v >> 16) & 0xFF,
				(v >> 8) & 0xFF, v & 0xFF);
	}
