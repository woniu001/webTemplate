...
public class SelfPermissionInterceptor implements HandlerInterceptor {
	...
	public boolean preHandle(HttpServletRequest request,
		...
		if (handler instanceof HandlerMethod) {
			...
			SelfPermission permission = getAnnotation(handlerMethod,
					SelfPermission.class);
			if (permission != null) {
				try{
	...
	public boolean checkPermission(final String permissionCode) throws Exception {
...
//			boolean result = IterableUtils.matchesAny(jrole.getPermissions(), new Predicate<Permission>() {
...
//			return result;
	...
	private <T extends Annotation> T getAnnotation(HandlerMethod handlerMethod,
		...
		annotation = AnnotationUtils.findAnnotation(
				handlerMethod.getBeanType(), clazz);
		return annotation;
	}
